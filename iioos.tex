...
paper, we present Online Outcome Sampling (OOS), 
%the first imperfect information 
%search algorithm that is guaranteed to converge to an equilibrium strategy in two-player 
%zero-sum games. 
an online search variant of the offline Monte Carlo Counterfactual Regret Minimization.
We show that OOS can avoid the common pitfalls encountered by existing 
search algorithms and we experimentally evaluate its convergence rate and head-to-head 
...
Computer Poker \cite{Sandholm10The,Rubin11Poker,Gilpin09,Johanson07Msc}.
...
Strategy fusion occurs when an algorithm recommends two different moves in two different states that the player should
not be able to distinguish (\ie due to hidden information not known by that player).
Fortunately, strategy fusion can be overcome by imposing the proper information constraints during search~\cite{Frank98Finding,Ciancarini10Kriegspiel,Ponsen11Computing,Lisy12peg,Cowling12ISMCTS}. 

Non-locality occurs due to optimal payoffs not being conveniently defined over subgames as they are 
in perfect information games. 
As such, it is fundamental problem that causes most formal guarantees of algorithms that exploit this subgame decomposition structure, such as minimax and Monte Carlo tree
search, to no longer hold in imperfect information games, despite any adaptations that fix strategy fusion.   
There is evidence that suggests that MCTS will not converge to the optimal solution even in very
...
Equilibrium strategies tend to be important in games with low {\it disambiguation factor}~\cite{Long10Understanding}, 
...
However, pre-computing an approximate equilibrium offline approach is often not possible. 
For example, an agent may be required to 
...
or in gaming apps on mobile devices with comstrained memory.
...
like Monte Carlo tree search (MCTS)~\cite{mctssurvey}. 
...
We propose two new ways to compute the exploitability of strategies produced by imperfect information search algorithms. 
We show the observed convergence rates and head-to-head performance of OOS in practice, comparing them to benchmark
