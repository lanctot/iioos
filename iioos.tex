...
MMCTS uses regret minimization at each information set in its Monte Carlo tree search, and scales the payoff 
...
for pre-computing approximate equilibria in abstract Poker games. 
...
There is a current \defword{match history}, $\tth$, initially set to 
the empty history $\emptyset$ representing the start of the match. On each turn, 
...
Every perfect information games can be recursively broken down into 
subgames and solved independently. 
...
A \defword{public action}, $a \in A(I)$, to be one where for all $h \in I$, $ha \in I'$. 
...
Depending on the method being used, define $Z_{sub}$ as $Z_{I(\tth)}$ or $Z_{p,I(\tth)}$. 
