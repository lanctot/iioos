...
Here, we define the relevant game-theoretic terminology for our analysis. The notation used here is based on \citeauthor{OsbRub94}~\shortcite{OsbRub94}. 
...
by player $i$ along $z$. We also use $\pi^{\sigma}_i(h,z)$ and $\pi^{\sigma}(h,z)$ to refer to the product 
...
For any $p \in (0, 1]$ and $\delta > 0$ there exists $t < \infty$ such that with probability $1-p$ the strategy  $\bar{\sigma}^t_m(\delta,\tth)$ is a $\delta$-equilibrium strategy. 
...
ISMCTS are locked in at $\approx$ 0.8, which we have also confirmed via the aggregate method.
