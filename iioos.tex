...
turn depend on the rewards obtained in other parts of the tree. Hence, the ``subgames'' cannot be solved independently 
...
\begin{center}
\begin{tabular}{cc}
\includegraphics[scale=0.6]{plots/ismcts-oos-perf} & 
\includegraphics[scale=0.6]{plots/eq-oos-perf} \\
\includegraphics[scale=0.6]{plots/oos-expl} &
\includegraphics[scale=0.6]{plots/goof-expl}
\end{tabular}
\caption{Results for OOS in LD(1,1), Top left: win rate (\%) of OOS vs. ISMCTS. Top right: win rate (\%) of 
OOS vs. a 0.001-equilibrium. Approximate $\epsilon_{\sigma}$ using aggregate method in LD(1,1) (bottom left)
and II-Goofpiel(6) (bottom-right).}
