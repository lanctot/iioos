...
\begin{algorithm2e}[t]
  ...
  \breturn $(x, l, u)$ \;     \label{alg:return}
  \popline
  \vspace{0.1cm}
...
However, in practice it will not sample $z \in Z_{sub}$ directly, but on a per-action basis. 
There are two stages, the prefix stage ($h \sqsubset \tth$), and the suffix stage ($\tth \sqsubseteq h$). 
In the prefix stage, $\cP_{sub}$ is a distribution 
that assigns some high probability $\gamma$ to sampling an action that is consistent with $\tth$. In the suffix case, the 
...
Lines~\ref{alg:suffix1} to \ref{alg:return} contain the usual outcome sampling updates. Note that regrets are updated at the 
