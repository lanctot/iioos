...
a \defword{multi-match} method: this simply runs several matches and saves all values from children of each $I$ visited during 
search, overwriting previous values as necessary. 
...
% Probably will ditch this multi-match method when partial stitching is working.
...
% multi-match method - 1 second
%
% ISMCTS(max)
% 
% ISMCTS(mix)
% matches played 15 15, avg. expl1 = 0.697268, exp2 = 0.799991, ttlAvgExpl = 1.49726
%
% OOS 
% matches played 30 30, avg. expl1 = 0.383926, exp2 = 0.625234, ttlAvgExpl = 1.00916
%

% multi-match method - 5 seconds
% ISMCTS(max)
% matches played 30 30, avg. expl1 = 0.687672, exp2 = 0.800723, ttlAvgExpl = 1.48839
...
\hline
ISMCTS(max) & 1s   & 0.670 & 0.800 & 1.470 \\
ISMCTS(mix) & 1s   & 0.659 & 0.797 & 1.456 \\
OOS         & 1s   & 0.384 & 0.625 & 1.009 \\
\hline
ISMCTS(mix) & 5s   &       &       &       \\
OOS         & 5s   &       &       &       \\
\hline
...
OOS         & 25s   &       &       &       \\
\end{tabular}
\caption{LD(1,1) exploitability using multi-match method.}
\end{center}
\end{table}
...
OOS           & 5s   &        &        &       \\
\end{tabular}
\caption{LD(1,1) exploitability using full-stitching method. \mlnote{I had some numbers but then found a bug; rerunning 1 second results. 
5 second results will take 68 hours {\tt :)}, will set to run as soon as 1s are done}} 
\end{center}
\end{table}
