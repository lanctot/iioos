...
A history $z \in Z$, where $Z \subseteq H$, is called a \defword{terminal history} represents a full game from start to end. 
...
if $h \sqsubset h'$ and $h' \sqsubset h''$ then $h \sqsubset h''$. Each set $N$, $H$, $Z$, and $A(h)$ for every $h \in H$ are 
...
$\pi^\sigma(z) = \prod_{i \in N} \pi_i(z)$, where each $\pi_i(z) = \prod_{ha \sqsubset z, P(h) = i} \sigma_i(h,a)$ 
...
A block set $\cQ$ is defined whose elements 
...
OS, a single $z \in Z$ is sampled and values at the information sets along the path to $z$ are modified. 
