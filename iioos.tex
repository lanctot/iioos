...
In this paper, we present Online Outcome Sampling (OOS), the first imperfect information search algorithm that is 
guaranteed to converge to an equilibrium strategy in a two-player zero-sum game.   
In addition, we show that OOS avoids common problems encountered by determinization.
We evaluate the practical performance and convergence rate of OOS and comparing it to benchmark strategies and 
to Information Set Monte Carlo Tree Search (ISMCTS).
We show that OOS performs at well as ISMCTS in head-to-head play while producing strategies with lower 
exploitability on two domains: Liar's Dice and an imperfect information variant of Goofspiel. 
Finally, we show that exploitability of strategies produced by OOS decreases as the amount of search time increases. 
\end{abstract}
