...
the amount of search time increases, and that Information Set Monte Carlo tree search (ISMCTS) can remain exploitable or even get more exploitable in time.
...
Algorithms for creating strong strategies in games have been a core interest of artificial intelligence research since its beginning. They are useful not only as research benchmarks and source of entertainment for people, but they are becoming increasingly important also in real world applications, such as critical infrastructure security \cite{tambesBook}.

Because these games are often played against unknown opponents, a good strategy should be sufficiently robust to withstand any possible counterstrategy that could be used by the adversary. This is particularly important in the security applications. In strictly competitive games, the strategy that is optimal with respect to the robustness against any opponent is the Nash equilibrium. It is guaranteed to provide the maximal possible expected value against any strategy of the opponent.

When preparation time abundant and the exact problem definition know in advance, an equilibrium strategy for a smaller abstract game can be pre-computed and then used during game play. This {\it offline approach} has been remarkably successful in 
Computer Poker \cite{Sandholm10The,Rubin11Poker,Gilpin09,Johanson07Msc,Johanson13Evaluating} and it is also prevalent used in the security applications \cite{}.
However, the preparation time is often very limited. The exact model of the game may become known only shortly before acting is necessary, such as in general game-playing, search or pursuit in a previously unknown environment, and general-purpose robotics. In a short time, creating and solving a sufficiently exact abstractions of the problem may not be possible. In these cases, agents need to {\it decide online}: make initial decisions quickly and then put additional effort to improving their strategy in the current situation while the interaction is taking place.

In this paper, we investigate how close to the Nash equilibirum strategy can we play a previously unknown game with a very limited computation time. We focus on zero-sum extensive-form imperfect-information games.
We introduce Online Outcome Sampling (OOS), a simulation-based algorithm based on Monte Carlo Counterfactual Regret Minimization~\cite{}, one of the most popular offline equilibrium solving algorithm for this class of games. In order to use computational time and space efficiently, we add to major modifications to the algorithm. First, OOS builds its search tree incrementally, like Monte Carlo tree search (MCTS)~\cite{Coulom06Efficient,UCT,mctssurvey}. Second, when performing additional computation after some moves have been played in the match, the following samples are targeted primary to the parts of the game tree, which are more relevant to the current situation in the game.
We show both formally and empirically that OOS is {\it consistent}, \ie it is guaranteed to converge to an equilibrium strategy as search time increases. To the best of our knowledge, this is not the case for any existing online game playing algorithm. OOS is the first algorithm that can overcome the problem of \emph{non-locality} of the optimal strategies in these games.

In the empirical analysis of OOS, we compare its convergence and head-to-head performance to Information Set MCTS~\cite{Cowling12ISMCTS,Whitehouse13Integrating, Lisy2014selection} in three fundamentally different games. 
In order to do that, we develop a novel methodology for evaluating convergence of online game playing algorithms that do not produce a complete strategy, but only select a move to play in each particular situation. The results show that OOS, unlike ISMCTS, consistently converges close to NE strategy in all three games. Furthermore, while ISMCTS can be substantially better in large games with very short computation times, longer computation times allow OOS to tie or even win the direct head-to-head matches.
...
In Section~\ref{sec:nonlocality}, we show a specific example of a game that demonstrate this problem and how it prevents the most common search algorithms to find the optimal solution in the game.

The last complication, sometimes disregarded by the algorithms for search in games, is the need for randomized strategies in imperfect information games. In many games, any deterministic strategy that can be predicted by the opponent can lead to arbitrarly large losses.
...
{\it diverge away} from the Nash equilibrium strategies.

A little more promissing are the MCTS approaches using alternative selection functions, such as Exp3 or Regret Matching \cite{Lisy2014selection}. 
Variants of MCTS with these selection functions have been shown both formally and empirically \cite{lisy2013nips, goofspielPaper} to converge to Nash equilibrium in games with simultaneous moves, which are the simplest subclass of imperfect information games. However, we show that even these selection functions cannot help to overcome the problem of non-locality, if the search is initiated form the current state of the game, as suggested by all existing search algorithms.
...
% Poker and the game-theoretic offline approach \vlnote{this can be possibly added to the related work}
%Billings04Game
...
%information that cause the current approaches not to converge to the optimal strategy. 
...
\end{tikzpicture}
\end{center}
\caption{An game demonstrating the problem of non-locality with maximizing $\bigtriangleup$, minimizing $\bigtriangledown$ and chance $\bigcirc$ players.
%The game of the left has perfect information, while the game on the right has imperfect information.
\label{fig:coordGame}}
\end{figure}
...
%as follows: the minimizing player$\bigtriangledown$ has two subgames with value 0 (since $0 < 3$), leading to a value of 0 for the maximizing player's larger subgame. The only other %subgame for $\bigtriangleup$ gives a value of $1$. Therefore, the expected value of the full game is $0.5 \cdot 1 + 0.5 \cdot 0 = 0.5$.
%\vlnote{this explains more strategy fusion than non-locality}
...
In order to prevent this effect, we add the weighting factor $w_T$ to compensate the change of targeting and make each of the samples have similar weight. In our example, $w_T=\frac{s'_0}{s_0}$. More formally, when running the iterations from match history $\mathtt{h}$, we define the weighting factor as:
\[w_T(\mathtt{h}) = (1-\delta) + \delta\frac{A}{B}\]
...
% it depends... but it make much easier to merge differences when more people are editing at once
...
\subsection{Information Set Monte Carlo Tree Search}
\vlnote{Describe ISMCTS, related work, use of empirical frequencies and default parameters}
...
\end{minipage}}
\subfigure[OOS noW Params.]{\label{fig:GS-oos}
\begin{minipage}{0.235\textwidth}
...
\end{minipage}}
\subfigure[OOS-IST Params.]{\label{fig:LD-oos}
\begin{minipage}{0.235\textwidth}
\includegraphics[width=\textwidth]{fig/agg_OSS-IST-WC_convergence-LD_6_1_1}
\end{minipage}}
\subfigure[OOS-PST Params.]{\label{fig:LD-oos}
\begin{minipage}{0.235\textwidth}
...
Figures~\ref{fig:small}(b,f,j) present the average exploitability of the strategies computed by different algorithms estimated by the aggregate method explained in Section~\ref{sec:evaluating}. The OOS in this graphs is run with the information set targeting and the parameters $\gamma=0.4$ for exploration and $\delta=0.5$ for targeting.
...
OOS-IST&&50.0(1.3)&50.5(1.3)&83.1(1.5)\\
RUCT&51.2(2.2)&62.9(2.6)&62.7(2.5)&84.0(2.1)\\
...
OOS-IST&&28.3(3.9)&35.1(4.2)&83.0(3.2)\\
...
\small
\begin{verbatim}
MCTS-RM_OOS_yIST-W_IIGS_1_6_t100 51.3(1.3)
MCTS-RUCT_OOS_yIST-W_IIGS_1_6_t100 50.8(1.2)
OOS_MCTS-RM_yIST-W_IIGS_1_6_t100 49.7(1.3)
OOS_MCTS-RUCT_yIST-W_IIGS_1_6_t100 50.8(1.3)
OOS_RAND_yIST-W_IIGS_1_6_t100 84.5(1.1)
RAND_OOS_yIST-W_IIGS_1_6_t100 15.4(1.1)

MCTS-RUCT_OOS_yIST-WC_IIGS_1_10_t3000 66.7(3.9)
OOS_MCTS-RUCT_yIST-WC_IIGS_1_10_t3000 41.1(4.2)
MCTS-RUCT_OOS_yIST-WC_IIGS_1_13_bFALSE_t5000 63.9(5.6)
OOS_MCTS-RUCT_yIST-WC_IIGS_1_13_bFALSE_t5000 34.8(5.8)
\end{verbatim}
...
\begin{small}
\begin{tabular}{@{}|@{}r@{}|@{}c@{~}c@{~}c@{~}c@{}|}\hline
0.1s&OOS-PST&UCT&RM&RND\\\hline
OOS-PST&48.5(2.0)&51.7(2.0)&56.8(2.8)&83.3(1.5)\\
...
OOS-PST&&45.2(3.1)&39.7(3.0)&89.3(1.9)\\
UCT&69.0(2.9)&48.2(4.0)&47.5(4.0)&85.8(2.8)\\
RM&65.7(2.9)&53.2(4.0)&52.7(4.0)&89.5(2.5)\\
RND&9.3(1.8)&15.3(2.9)&16.0(2.9)&49.3(4.0)\\
\hline
\end{tabular}
\end{small}
}
...
\begin{small}
\begin{tabular}{@{}|@{}r@{}|@{~}c@{~}c@{~}c@{~}c@{}|}\hline
5s&OOS-PST&UCT&RM&RND\\\hline
OOS-PST($\gamma$=0.8)&49.9(3.7)&55.4(2.3)&53.3(3.7)&91.3(1.7)\\
...
%\subfigure[Generic Poker (3,3,2,2) $\pm$0.84]{\label{fig:GP-matches}
%\begin{small}
...
\subfigure[Generic Poker (3,3,2,2)]{\label{fig:GP-matches}
\begin{small}
\begin{tabular}{@{}|@{}r@{}|@{\hspace{1pt}}c@{~}c@{~}c@{~}c@{}|}\hline
0.1s&OOS-PST&UCT&RM&RND\\\hline
OOS-PST&0.25(0.53)&-0.68(0.35)&-0.39(0.33)&2.00(0.54)\\
UCT&0.64(0.37)&0.21(0.15)&0.20(0.15)&1.35(0.36)\\
RM&0.43(0.54)&0.21(0.40)&-0.12(0.32)&1.75(0.44)\\
RND&-2.05(0.70)&-2.22(0.54)&-3.24(0.55)&1.07(0.61)\\
\hline
\end{tabular}
\end{small}
}
...
\begin{tabular}{@{}|@{}r@{}|@{~}c@{~}c@{~}c@{~}c@{}|}\hline
1s&OOS&UCT&RM&RND\\\hline
%OOS(5s)&&-0.85(0.46)&&\\ %UCT-OOS 0.40(0.60)
OOS-IST&&0.03(0.62)&-0.57(0.66)&5.83(1.41)\\
UCT&0.61(0.89)&-1.10(1.17)&-1.05(0.90)&8.02(1.89)\\
RM&-0.63(0.97)&-1.35(1.24)&-0.32(0.92)&8.94(1.87)\\
RND&-4.64(1.28)&-4.44(1.40)&-4.67(1.35)&0.46(2.19)\\
\hline
\end{tabular}
\end{small}
}
...
For Goofspiel and Liar's Dice, we present the win-rate, where a tie is considered half win half loss. Goofspiel is perfectly balanced for both players. In LD, the first player has a slight disadvantage corresponding to approximately 1% in the win rate and we do not compensate for that.
