...
Non-locality, however, is a fundamental problem that prevents the usual minimax formulation of optimal payoffs defined strictly over 
children subtrees. 
There is empirical evidence suggesting that some of these techniques will not converge to the optimal solution even in very
...
several weeks or even months. In the offline approach, during actual play an agent simply looks up its precomputed strategy 
...
In this paper, we focus on the {\it online} setting. We propose a  Monte Carlo Tree Search (MCTS)~\cite{mctssurvey} algorithm that 
...
known imperfect information search algorithm satisfying this property, which we call {\it consistency}. 
...
We show the empirical convergence rates and performance of OOS, comparing them to benchmark players and to ISMCTS~\cite{Cowling12ISMCTS}, a 
...
Ultimately, we would like to make practical comparisons to other baseline algorithms such as 
