...
In this paper, we focus on the {\it online} setting. In essence, we propose a Monte Carlo Tree Search algorithm that 
uses Monte Carlo Counterfactual Regret Minimization as a basis rather than UCT.
...
A \defword{public action} is an action in the ``public tree'' defined in \cite{johanson2012efficient}. Informally, an action is said to be public if it is observable by all players (e.g., a bid in Liar's Dice or Poker is public). Formally, an action $a$ is public, iff 
