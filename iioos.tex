...
as needed from $\sigma_c(\tth)$, and reveales $I(\tth)$ to $P(\tth)$ on their turn. The players play until the match is terminated, 
...
Search algorithms such as minimax and MCTS simply aim to identify the single optimal action at the search tree's root. 
...
Consequently by \cite[Theorem 5]{Lanctot09Sampling} OOS mininimizes external regret with high probability.
...
Both players discard the card they used to bid.  
...
\begin{figure*}[t]
\begin{tabular}{ccc}
\includegraphics[scale=0.45]{plots/ismcts-oos-perf} & 
\includegraphics[scale=0.45]{plots/eq-oos-perf} &
\includegraphics[scale=0.45]{plots/oos-expl}
\end{tabular}
\caption{Results for OOS in LD(1,1) for $\delta \in \{ 0, 0.1, 0.5, 0.65, 0.8, 0.9, 0.95, 0.99, 1 \}$. 
Left: win rate (\%) of OOS vs. ISMCTS. Middle: Left: win rate (\%) of OOS vs. a 0.001-equilibrium. 
Right: approximate $\epsilon_{\sigma}$ using aggregate method.}
\label{fig:ld11-results}
\end{figure*}
...
a partial strategy. 
The first \defword{full stitching} method tries to capture as closely as possible the distributions that would be computed 
for each $I$ if they were reached in a match: it enumerates each $I \in \cI_i$ in topologically-sorted order starting at the root, 
...
Full-stitching is computationally intensive, requiring $|\cI|$ searches and memory, which is impractical on large games. 
Therefore, we also propose another method to approximate the exploitability for larger games: the multi-match 
\defword{aggregate method}. 
This method first resets a global (accumulating) strategy data structure for each player type and generates a batch of matches $M$.
Then, each $m \in M$ is simulated as if it were a normal game being played (where each player starts with empty strategies)
and invokes the appropriate search algorithm at each observed $I$ along $m$. 
Since $m$ is predetermined, the choice made by the search algorithm is discarded, but the information computed is 
{\it aggregated} into the global data structure for that player type $P(I)$. In the case of ISMCTS, visit counts for actions at 
$I$ are incremented by the ones searches. For OOS, the values in the average strategy tables $s_I$ are incremented.
...
In our experiments ISMCTS uses $C = 2$ and tuning shows that $C$ value does not 
affect the performance of ISMCTS. 
In Goofspiel we run searches for 1, 2, and 4 seconds.
In LD(1,1), we try searches for 1, 5, and 25 seconds. 
Among all values for delta and time settinfs, IST wins 79.2-86.7\% and PST wins 78.6-84.0\%, with a 
...
Upon further inspection, ISMCTS very quickly converges to the same strategy each time: as first player, 
...
Results OOS variants on LD(1,1) are shown in Figure~\ref{fig:ld11-results}. Firstly, in head-to-head performance 
...
increases with $\delta$, except when $\delta = 1$ which we suspect is due to benefiting from the reuse of values 
from previous search in the same match. PST is less affected than IST by the targeting and appears to have lower exploitability. 
...
Finally, we performed one last head-to-head experiment of 500 games, with 25 seconds of search time of 
PST($\delta = 0.8$) against ISMCTS in LD(2,2). PST won 256 of these, showing a competitive promise on
...
%
% PST 
% Done. v1 (br to fixed p2) = 0.148224
% Done. v2 (br to fixed p1) = 0.124856
% 
%
% ISMCTS
...
For future work, we aim to derive finite-time guarantees 
for the resulting strategies and investigate the application to 
larger such as no-limit Poker. 
Finally, we plan to try OOS with MCRNR~\cite{Ponsen11Computing} as
the base strategy to produce a search player that balance 
exploitability and exploitation versus known opponents.
