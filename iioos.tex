...
In Poker, CFR has been used with remarkable success as an offline technique. Essentially, CFR takes time (weeks?) {\it pre-computing} 
...
to decide on an action $\tta \in A(\tth)$ and the 
...
So far, the only variant of OOS implemented is IST with $(\epsilon, \gamma) = (0.6, 0.97)$. Manual tuning shows
that $\gamma > 0.9$ is a good region. ISMCTS uses $C = 2$ and manual tuning shows that the value of $C$ does not 
...
I have run the single-match method for a small number of games, and averaged the results 
