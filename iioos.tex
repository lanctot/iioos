...
OOS is shown to outperform its competitor in randomly generated game trees as well as two complex domains: Liar's Dice and 
Phantom Tic-Tac-Toe. 
\end{abstract}
...
In GIB, game tree search is performed by running perfection information search on a {\it determinized} 
...
There are several problems that have been identified with performing Perfect information Monte Carlo (PIMC) search techniques
over determinized sampled. The most common are {\it strategy fusion} and {\it non-locality}~\cite{Frank98Finding}.
...
Strategy fusion can be alleviated by imposing the proper information constraints during search, as was done in 
Kriegspiel~\cite{Ciancarini10Kriegspiel}, Kuhn Poker~\cite{Ponsen11Computing}, pursuit-evasion games~\cite{Lisy12peg}, 
and thoroughly explained in~\cite{Cowling12ISMCTS}. 
Non-locality, however, is a fundamental problem that prevents PIMC approaches from being consistent.
Consequently, the usual minimax-style backup rule is not always applicable and can lead to overly paranoid 
behavior at opponent nodes. 
In the work of~\cite{Parker10iss,Parker06Overconfidence}, the authors show that an overconfident 
backup rule can perform better in practice. 
We show that ISMCTS also suffers from this problem as well. 

% other missing related works

One particularly related work is the mulitple tree (MMCTS) algorithm~\cite{Auger11Multiple} that has been 
applied to Phantom Tic-Tac-Toe. 
MMCTS uses Exp3 at each information set in its Monte Carlo tree search, and scales the payoff received by the 
depth of each playout. While we have not implemented MMCTS, we have been able to test our algorithm 
against it thanks to the authors sharing their code. 
...
algorithms to compute a near-optimal equilibrium of an abstract game, and then use the abstract strategies 
when playing the full game~\cite{Sandholm10The}. This {\it offline} approach requires designing 
abstractions and pre-computing Poker equlibria, which is often not possible. Also, due to the size of 
abstractions~\cite{Johanson13Evaluating}, the memory resource may not be available or known in advance. 
...
In Poker, CFR has been used with remarkable success as an offline technique for pre-computing approximate equilibria for 
abstract Poker games. 
...
the empty history $\emptyset$ representing the start of the match. On each turn, the agent controlling $P(\tth)$ is given $t$ seconds 
to decide on an \defword{match action} $\tta \in A(\tth)$ and the 
...
%Preliminary results shown in Figure~\ref{fig:os-vs-pbos} seem to indicate 
...
The main difficulty remaining is to deciding how to direct the search when it is invoked {\it during the match}, 
...
\mlnote{Need to reformulate as new version.}
...
\[Z_{p,I(h)} = \{ z | z \in Z, h \in I, h' \in H, p(h') = p(h), h' \sqsubset z \}\]
(Need to define this better too; also, note that this is not a true subgame in the usual game-theoretic sense.. so should we call it 
something else?)
...
\begin{algorithm2e}[t]
  OOS$(h, \pi_i, \pi_{-i}, s, i)$: \;
  \pushline
  \lIf{$h \in Z$}{\breturn $(1, s, u_i(z))$}
  $I \gets $ getInfoset$(h, P(h))$ \;
  Sample action $a$ with pr. $\rho$ from $\cP_{sub}(h, I, i, \gamma, \epsilon)$ \; \label{alg:sample}
  \If{$P(h) = c$}{
    \breturn OOS$(ha, \pi_i, \sigma_c(h,a) \pi_{-i}, \rho s, i)$ \;
  }
  ...
  \Else{
    ...
    $(x, l, u) \gets$ OOS$(ha, \pi_i', \pi_{-i}', \rho s, i)$ \;  
  }
  $c \gets x$ \;                   \label{alg:suffix1} 
  $x \gets x \sigma(I,a)$ \;     \label{alg:suffix2}
  \For{$a' \in A(I)$}{   
    \If{$P(h) = i$}{
      $W \gets \frac{u \pi_{-i}}{l}$ \;
      \lIf{$a' = a$}{$r_I[a'] \gets r_I[a'] + (c - x)W$}
      \lElse{$r_I[a'] \gets r_I[a'] - xW$}
    }
    \Else{
      $s_I[a] \gets s_I[a] + \frac{1}{s} \pi_{-i} \sigma(I,a)$ \;  \label{alg:avgstrat}
    }
  } 
...
\subsection{Preliminary Results in Bluff(1,1)}
...
%    1s      & OOS wins 59.10 $\pm$ 3.05\% of games \\
%    5s      & OOS wins 54.70 $\pm$ 3.05\% of games \\
%    25s     & OOS wins 54.80 $\pm$ 3.05\% of games \\
    1s      & OOS wins 61.20 $\pm$ 3.02\% of games \\
    5s      & OOS wins 58.60 $\pm$ 3.05\% of games \\
    25s     & OOS wins 55.30 $\pm$ 3.08\% of games \\
\hline
\end{tabular}
\caption{OOS vs. ISMCTS head-to-head performance on full games.}
\end{table}
\end{center}

There are several ways that the expoitability of the strategies produced by these search algorithms can be evaluated 
(in each case $X \in \{$ OOS, ISMCTS $\}$ and $B$ is some fixed benchmark strategy):
\begin{itemize}
\item The {\bf single-match method}. Runs algorithm $X$ vs $B$ for a single match.
      Then compute the exploitability of the strategy computed by $X$. 
\item The {\bf multi-match method}. Run the single-match method for $M$ matches and save each full strategy. Then, compute the 
      average strategy $\bar{\sigma}^M$ over all matches. Compute the exploitability of $\bar{\sigma}^M$.
\item The {\bf partial-stitching method}. For all $I \in \cI_1$: run the search algorithm $X$ starting at $I$ and starting with 
      empty strategy (nothing saved from previous match decisions). After each search, save the distribution $\bar{\sigma}(I)$ 
      computed by $X$ to a global ``stitched'' strategy $\bar{\sigma}'$. Compute the exploitability of $\bar{\sigma}'$.      
\item The {\bf full-stitching method}. For all $I \in \cI_1$: run a search at each $I'$ which is a predecessor of $I$ retaining
      memory. Then, run a search from $I$, starting with a strategy computed from these previous searches. 
      As in partial stitching, save the distribution computed at $I$ to a 
      globally stitched strategy $\bar{\sigma}'$ and compute the exploitability of the stitched strategy.
\end{itemize}

I have run the single-match method for a small number of matches (5 per data point), and averaged the results 

\begin{center}
\begin{tabular}{ccccc}
alg & time & avg. exp(1) & avg. exp(2) & total avg. exploitability\\
\hline
ISMCTS & 1s & 0.795 & 0.767 & 1.562 \\
% Before the algorithm change:
% OOS    & 1s & 0.703 & 0.444 & 1.147 \\
OOS    & 1s & 0.629 & 0.453 & 1.082 \\
\hline
ISMCTS & 5s & 0.796 & 0.768 & 1.564 \\
% Before the algorithm change:
%OOS    & 5s & 0.518 & 0.283 & 0.801 \\
