...
void getInfoset(unsigned long long & infosetkey, Infoset & is, GameState & gs, unsigned long long bidseq, int actionshere) {
}

double cfroos(GameState & gs, int player, int depth, unsigned long long bidseq, 
              ...
              double & suffixreach, double & rtlSampleProb, bool treePhase)
{
  ...
  if (gs.p1roll == 0) 
  {
    ...
    return cfroos(ngs, player, depth+1, bidseq, reach1, reach2, sprob1, sprob2, updatePlayer, suffixreach, rtlSampleProb, treePhase); 
  }
  else if (gs.p2roll == 0)
  {
    GameState ngs = gs; 
  ...
  updatePlayerPayoff = cfroos(ngs, 3-player, depth+1, newbidseq, newreach1, newreach2, newsprob1, newsprob2, 
