 ...
 */
#ifndef __SEQSTORE_H__
#define __SEQSTORE_H__
...
class SequenceStore
{
  ...
  bool addingEntries;
  unsigned long long nextEntryPos;
  unsigned long long added;

  bool get_priv(unsigned long long sskey, SSEntry & ssentry, int moves, int firstmove); 
  void put_priv(unsigned long long sskey, SSEntry & ssentry, int moves, int firstmove); 
  
  bool get_priv(unsigned long long sskey, Infoset & infoset, int moves, int firstmove, int chanceOutcome, bool regretmatching = true); 
  void put_priv(unsigned long long sskey, Infoset & infoset, int moves, int firstmove, int chanceOutcome); 
...
public:
  ...
  void stopAdding() { addingEntries = false; } 
  unsigned long long getNextPos() { return nextEntryPos; }
  unsigned long long getAdded() { return added; }
  
  std::string getStats();

  bool get(unsigned long long sskey, SSEntry & ssentry, int moves, int firstmove); 
  void put(unsigned long long sskey, SSEntry & ssentry, int moves, int firstmove); 
  bool get(unsigned long long sskey, Infoset & infoset, int moves, int firstmove, int chanceOutcome, bool regretmatching = true); 
  void put(unsigned long long sskey, Infoset & infoset, int moves, int firstmove, int chanceOutcome); 
  
  bool fsiIncReaches(unsigned long long sskey, int moves, int firstmove, int chanceOutcome, double amount1, double amount2); 
  bool fsiGetValue(unsigned long long sskey, double & value, int moves, int firstmove, int chanceOutcome); 
  ...
  // If we need these back, see infosetstore.cpp
  //
  //void importValues(int player, std::string filename);
  // abstraction-specific functions
  //void absComputeBound(SequenceStore & abs_iss, double & sum_RTimm1, double & sum_RTimm2); 
  //void absImportStrategy(SequenceStore & abs_iss); 

  // used by MCTS
  void clear(); 
  void copy(SequenceStore & dest);
